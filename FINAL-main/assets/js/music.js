...
$(window).scroll( () => {
  if ($(".music-container").is(":visible") || x != 0) {
    x += 1;
    if (
...
// Song titles
const songs = ["hey", "summer", "Syrlasu Valsi", "ukulele"];
const disk = ["disk"];
...
function loadSong(song) {
  ...
  cover.src = `assets/images/${disk[0]}.png`;
}
...
$(document).keydown(function (e) {
  //For any other keypress event
  if (e.which == 188 && e.altKey) {
    prevSong();
  }
});
$(document).keydown(function (e) {
  //For any other keypress event
  if (e.which == 190 && e.altKey) {
    nextSong();
  }
});
